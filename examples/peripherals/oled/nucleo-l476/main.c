...
/*---------------------------- I2Cx TIMINGR Configuration ------------------*/
	...
	I2C_DEV->TIMINGR &= ~I2C_TIMINGR_PRESC; // Clear the prescaler 
	I2C_DEV->TIMINGR |= 7U << 28;           // Set clock prescaler to 7
	// tSCL = tSYNC1 + tSYNC2 + {[(SCLH+1) + (SCLL+1)] x (PRESC+1) x tI2CCLK}
	// SCL Frequency = 8 MHz / 100 = 80 kHz
	// Configure SCL high, low period
	I2C_DEV->TIMINGR |= 49U;       // SCLL: SCL low period (master mode) > 4.7 us
	I2C_DEV->TIMINGR |= 49U << 8;  // SCLH: SCL high period (master mode) > 4.0 us	 
	// Configure SDA setup, and hold time
	I2C_DEV->TIMINGR |= 1U << 20;  // SCLDEL: Data setup time > 1.0 us
	I2C_DEV->TIMINGR |= 2U << 16;  // SDADEL: Data hold time  > 1.25 us	
	...
	I2C_DEV->OAR1 &= ~I2C_OAR1_OA1EN;
	I2C_DEV->OAR1 = I2C_OAR1_OA1EN | OLED_I2C_DEV_ADDR; // 7-bit own address
	// 10-bit address mode
	// I2Cx->OAR1 = I2C_OAR1_OA1EN | I2C_OAR1_OA1MODE | OwnAddr; 
	
	// Enable clock stretching
	I2C_DEV->CR1 &= ~I2C_CR1_NOSTRETCH;
		//--------------------------------------------------
  ...
  //	MODIFY_REG(I2C_DEV->CR1, (I2C_CR1_NOSTRETCH), (I2C_GENERALCALL_DISABLE| I2C_NOSTRETCH_DISABLE));
	...
	I2C_DEV->OAR1 &= ~I2C_OAR2_OA2EN;
	
	/*---------------------------- I2Cx CR2 Configuration ----------------------*/
	I2C_DEV->CR2 &= ~I2C_CR2_ADD10;  // 0 = 7-bit mode, 1 = 10-bit mode
