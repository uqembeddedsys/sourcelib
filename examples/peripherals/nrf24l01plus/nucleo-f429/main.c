...
int main(void) {
	...
	while (1) {
		...
		nrf24l01plus_hal_wr(NRF24L01P_RF_CH, 40);			//Write 40 to the radio channel register
		debug_log("nrf24L01plus Channel register value: %#.2x\r\n", nrf24l01plus_hal_rr(NRF24L01P_RF_CH));	// See page 59 of nrf24L01+ datasheet for status register definition

		status = nrf24l01plus_hal_rr(NRF24L01P_STATUS);							//read the status register
...
void hardware_init(void) {
	...
	nrf24l01plus_hal_init();		//Initialise NRF24L01plus module (includes CE, CSN, MISO, MOSI & SCK pins)
